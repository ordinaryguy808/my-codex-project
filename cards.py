...
class ClosingConnection(sqlite3.Connection):
    """Commit or roll back and close when used as a context manager."""
...
class SQLiteCardRepository:
    ...
    @staticmethod
    def _to_request(row: sqlite3.Row) -> CustomerRequest:
        return CustomerRequest(**dict(row))

    def create_customer_request(self, **values: str) -> CustomerRequest:
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                (request_id, customer_name, business_name, email, phone, product_type,
                 destination_type, destination_url, design_option, uploaded_file_path,
                 design_notes, status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'NEW', ?)
                """,
                (
                    request_id,
                    values["customer_name"], values.get("business_name", ""), values["email"],
                    values.get("phone", ""), product_type, values["destination_type"],
                    values["destination_url"], values["design_option"],
                    values.get("uploaded_file_path", ""), values.get("design_notes", ""), created_at,
                ),
            )
            row = connection.execute(
    ...
    def list_customer_requests(self) -> list[CustomerRequest]:
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT * FROM customer_requests ORDER BY created_at DESC"
            ).fetchall()
        return [self._to_request(row) for row in rows]

    def get_customer_request(self, request_id: str) -> CustomerRequest | None:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM customer_requests WHERE request_id = ?", (request_id,)
            ).fetchone()
    ...
    def update_request_status(self, request_id: str, status: str) -> CustomerRequest | None:
